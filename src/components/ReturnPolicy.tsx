...
export const ReturnPolicy: React.FC = () => {
  ...
  const [vectorFilter, setVectorFilter] = useState<'ALL' | 'VECTOR_1' | 'VECTOR_2' | 'VECTOR_3' | 'VERIFIED'>('ALL');
  ...
  const vectorStats = useMemo(() => {
    ...
    returnParcels.forEach(p => {
      ...
      // Vector 2: Courier Hub Transit Retention
      if (p.courierReportedStatus === 'RETURNED' && !p.scannedAtWarehouse && p.deltaDaysInTransit > retentionThresholdDays) {
        v2Count++;
        v2TotalRiskBDT += p.parcelValueBDT;
      }

      // Vector 3: Ghost Return
      if (p.returnFeeBDT > 0 && !p.scannedAtWarehouse) {
        v3Count++;
  ...
  const filteredParcels = useMemo(() => {
    return returnParcels.filter(parcel => {
      ...
      if (vectorFilter === 'VECTOR_3') {
        const isGhost = parcel.returnFeeBDT > 0 && !parcel.scannedAtWarehouse;
            ...
            <div>
              <div className="flex items-center justify-between">
                <span className="text-xs font-mono font-bold text-[#EF4444] uppercase tracking-wide">
                  Vector 1: Penalty Cap Overcharge
                </span>
            ...
            <div>
              <div className="flex items-center justify-between">
                <span className="text-xs font-mono font-bold text-[#FACC15] uppercase tracking-wide">
                  Vector 2: Hub Transit Retention
                </span>
            ...
            <div>
              <div className="flex items-center justify-between">
                <span className="text-xs font-mono font-bold text-[#A855F7] uppercase tracking-wide">
                  Vector 3: Ghost Return (Zero-Debit)
                </span>
              ...
              <p className="text-[11px] text-[#A1A1AA] mt-1 font-sans">
                Courier reverse fees blocked from settlement until verified at gate.
              </p>
            </div>
            <div className="text-[10px] font-mono text-[#A855F7] bg-[#A855F7]/10 p-2 rounded border border-[#A855F7]/20">
              Rule: Courier Return Fee &gt; 0 &amp; Floor Scan Absent → 100% Payout Block
            </div>
          </div>
        </div>
      ...
      <div className="bg-[#121212] border border-[#27272A] rounded-2xl overflow-hidden shadow-xl">
        ...
        <div className="block sm:hidden divide-y divide-[#27272A]">
          {filteredParcels.map(parcel => {
            ...
            const isV3Ghost = parcel.returnFeeBDT > 0 && !parcel.scannedAtWarehouse;
            const isV1Breach = (parcel.returnFeeBDT - configuredBaseFeeBDT) > 0;
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-2 text-xs font-mono">
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-[#EF4444]">Ret Fee: {formatBDT(parcel.returnFeeBDT)}</div>
                  </div>
                </div>
